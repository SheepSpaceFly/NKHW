...
struct ivf_pq_parm{
    ...
    int *candidate_queries;
    float **residual;
    int *residual_i;
    int thread_number;
    int thread_id;
    std::priority_queue<std::pair<float, uint32_t> > *my_q;
    sem_t *sem_thread;
};
...
void* ivfpq_pq_thread(void* _parm){
    ...
    int size_all = parm->size_all;
    int *residual_i = parm->residual_i;
    pthread_mutex_t *amutex = parm->amutex;
    std::priority_queue<std::pair<float, uint32_t> > *main_q = parm->main_q;
    int *candidate_queries = parm->candidate_queries;
    float **residual = parm->residual;
    int thread_number = parm->thread_number;
    int thread_id = parm->thread_id;
    std::priority_queue<std::pair<float, uint32_t> > *my_q = parm->my_q;
    ...
    for(int i=my_first;i<my_last;i++){
        ...
        if(my_q->size() < k_top) {
            my_q->push({dis, candidate_query});
        } else {
            if(dis < my_q->top().first) {
                my_q->push({dis, candidate_query});
                my_q->pop();
            }
        }
    ...
    while (my_q->size()) {   
        if(main_q->size() < k_top) {
            main_q->push(my_q->top());
        } else {
            if(my_q->top().first < main_q->top().first) {
                main_q->pop();
                main_q->push(my_q->top());
            }
        }
