...
void* sq_search_thread(void* _parm){
    ...
    sq_thread_parm* parm = (sq_thread_parm*)_parm;
    uint8_t* base = parm->base;
    uint8_t* query = parm->query;
    int base_number = (int)parm->base_number;
    ...
    int chunk = (base_number+thread_number-1)/thread_number;
    int my_first = chunk*thread_id;
    int my_last = std::min(my_first+chunk,base_number);
