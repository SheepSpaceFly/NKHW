...
std::priority_queue<std::pair<float, uint32_t>> ivfpq_search(uint8_t* compressed_base, float *centers, float *ivf_centers,std::vector<int> *inverted_kmeans, float* query, size_t base_number, size_t vecdim, size_t k_top, int m, int thread_number) {
    ...
    static ThreadPool pool(thread_number);
    pthread_mutex_t amutex;
    pthread_mutex_init(&amutex, NULL);
    sem_t sem_thread;
    sem_init(&sem_thread, 0, 0);
    
    const int total_ivf = 256;
    ...
    for(int thread_id=0;thread_id<thread_number;thread_id++){
        pool.enqueue([&,thread_id] {
            ...
            for(int i = my_first; i < my_last; i++) {
                float dis = get_query_dis(ivf_centers+i*vecdim, query, vecdim);
                if(my_q[thread_id].size() < m) {
                    my_q[thread_id].push({dis, i});
                } else {
                    if(dis < my_q[thread_id].top().first) {
                        my_q[thread_id].push({dis, i});
                        my_q[thread_id].pop();
                    }
                }
            ...
            while (my_q[thread_id].size()) {   
                if(ivf_q.size() < m) {
                    ivf_q.push(my_q[thread_id].top());
                } else {
                    if(my_q[thread_id].top().first < ivf_q.top().first) {
                        ivf_q.pop();
                        ivf_q.push(my_q[thread_id].top());
                    }
                }
    ...
    int size_all = 0;
    std::vector<int> candidate_queries;
    std::vector<int> residual_i;
    ...
    for(int i=0; i<m; i++){
        ...
        for(int j=0;j<size;j++){
            candidate_queries.push_back((*candidate_base)[j]);
            residual_i.push_back(i);
        }
    ...
    for(int thread_id=0;thread_id<thread_number;thread_id++){
        pool.enqueue([&,thread_id] {
            int chunk = (size_all+thread_number-1)/thread_number;
            int my_first = chunk*thread_id;
            int my_last = std::min(my_first+chunk,size_all);
            ...
            for(int i=my_first;i<my_last;i++){
                ...
                if(my_q[thread_id].size() < k_top) {
                    my_q[thread_id].push({dis, candidate_query});
                } else {
                    if(dis < my_q[thread_id].top().first) {
                        my_q[thread_id].push({dis, candidate_query});
                        my_q[thread_id].pop();
            ...
            while (my_q[thread_id].size()) {   
                if(main_q.size() < k_top) {
                    main_q.push(my_q[thread_id].top());
                } else {
                    if(my_q[thread_id].top().first < main_q.top().first) {
                        main_q.pop();
                        main_q.push(my_q[thread_id].top());
                    }
                }
