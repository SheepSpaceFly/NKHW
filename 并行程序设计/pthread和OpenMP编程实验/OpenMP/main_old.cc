...
int main(int argc, char *argv[])
{
    ...
    // 构建pq_kmeans
    // kmeans_compress_base(base, base_number, vecdim);

    //读取pq_kmeans
    float *centers = new float[256*4*vecdim];
    uint8_t *compressed_base = new uint8_t[base_number*4];
    float *centers_dis = new float[4*256*256];
    read_kmeans(centers, compressed_base, centers_dis, base_number, vecdim);
    ...
    // build_inverted_kmeans(labels, centers, base_number, vecdim);

    // 读取倒排索引
    std::vector<int> inverted_kmeans[256];
    float* ivf_centers = new float[256*vecdim];
    read_inverted_kmeans(inverted_kmeans, ivf_centers, vecdim);
    ...
    for(int i = 0; i < test_number; ++i) {
        ...
        //ivfpq搜索
        auto res = ivfpq_search(compressed_base, centers, centers_dis, ivf_centers, inverted_kmeans, test_query+i*vecdim, base_number, vecdim, k);
