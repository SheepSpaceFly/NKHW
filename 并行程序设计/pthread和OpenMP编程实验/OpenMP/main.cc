...
int main(int argc, char *argv[])
{
    ...
    for(int i = 0; i < test_number; ++i) {
        ...
        //暴力搜索
        // auto res = flat_search(base, test_query + i*vecdim, base_number, vecdim, rerank_k);

        //sq搜索
        uint8_t *compressed_query = new uint8_t[vecdim];
        sq_compress_base(test_query+i*vecdim, compressed_query, 1, vecdim);
        auto raw_res = sq_search_openmp(compressed_base, compressed_query, base_number, vecdim, rerank_k, thread_number);
        ...
        //ivfpq搜索
        // int m=8;
        // auto raw_res = ivfpq_search(ivfpq_compressed_base, ivfpq_centers, ivf_centers, inverted_kmeans, test_query+i*vecdim, base_number, vecdim, rerank_k, m, thread_number);
